...
void add_employees(){
    ...
    sort_employees(0,MAX_EMPLOYEES);
}
// ----- Creates Teams -----
void create_team(){
...
void team_achievement(int team_ID){
    ...
    int total_hours = calculate_total_hours_team(team_ID); // Total Hour of the team
    ...
    if (choice == 'U' || choice == 'u'){ // for update
       if (emp_indices != -1 && emp_id != -1){
         ...
         if (emp_indices != -1 && emp_id != -1){
            ...
            employees[emp_indices].team_id = EMPTY_TEAM_ID;
...
void delete_team(int team_id){
    ...
    if(teamIndex == -1){
        ...
        sort_teams(0,num_of_teams);
        num_of_teams--;
        printf("\n TEAM with ID %d deleted SUCCESSFULLY. \n",team_id);
    }
}
// ----- Merge Team -----
void merge_team(int team_id1, int team_id2){
...
int load_employee_details_from_file() {
    ...
    char line[MAX_SIZE_EMPLOYEE_STRUCT + 15];
    int count = 0;
    int lineptr = 1;
...
int load_team_details_from_file() {
    ...
    char line[MAX_SIZE_TEAM_STRUCT+11]; 
    int count = 0;
    int lineptr = 1;
